  ...
  function initialize() {
    ...
    for (var i = 0; i < elements.length; i++) {
      if (elements[i].hasAttribute('data-src')) {
        console.error(
          "Animations no longer support 'data-src'! Use 'loadcontent' plugin instead."
        );
      }
  ...
  function seek(timestamp) {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      if (
        ...
        animatedSVGs[index].animation.time(
          animatedSVGs[index].animationSchedule[fragment].begin +
            (timestamp || 0)
        );
      }
    }
  ...
  function setupPrintout() {
    ...
    for (var i = 0; i < pages.length; i++) {
      ...
      for (var j = 0; j < elements.length; j++) {
        ...
        var fragments = elements[j].querySelectorAll(
          'svg [data-fragment-index]'
        );
        ...
        elements[j].innerHTML = elements[j].innerHTML.replace(
          /<\!--.*?-->/g,
          ''
        );
      }
